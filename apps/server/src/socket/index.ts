...
export function setupSocketHandlers(io: Server, db: Database, roomManager: RoomManager) {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('room:join', async (payload, cb) => {
      ...
      const detail = await roomManager.joinRoom(parsed.data.roomId, user, socket.id);
      if (!detail) {
    ...
    socket.on('room:leave', async () => {
      ...
      if (roomId) {
        ...
        if (detail) io.to(roomId).emit('room:updated', detail);
        const lobbyRooms = await roomManager.listRooms();
        io.emit('lobby:rooms', lobbyRooms);
      }
    });
    ...
    socket.on('game:start', async (_payload, cb) => {
      ...
      if (!result) {
        cb?.({ ok: false, message: '无法开始游戏，请检查人数与队列' });
        return;
      }
    ...
    socket.on('game:davinci:play', async (payload, cb) => {
      const roomId = getRoomId(socket);
      if (!roomId) return;
      const member = await findMember(roomManager, roomId, user.id);
      if (!member) return;

      const game = await roomManager.processDaVinciMove(
        roomId,
        member.id,
...
async function processBots(io: Server, roomManager: RoomManager, roomId: string) {
  let safety = 0;
  while (safety < 20) {
    const before = roomManager.getGame(roomId);
    const after = await roomManager.runBotTurns(roomId);
    if (!after || JSON.stringify(before?.state) === JSON.stringify(after.state)) break;
    io.to(roomId).emit('game:state', { gameType: after.gameType, state: after.state });
    safety++;
  }
}
