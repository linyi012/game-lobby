  ...
  return (
    <div className="card" style={{ display: 'flex', flexDirection: 'column', minHeight: 200 }}>
      ...
      {canGuess ? (
        ...
        >
          <input className="input" name="guess" placeholder="提交你的猜测…" autoComplete="off" />
          <button type="submit" className="btn">
            发送
          </button>
        </form>
      ) : (
        <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', margin: 0 }}>{disabledHint}</p>
      )}
    </div>
  );
}
